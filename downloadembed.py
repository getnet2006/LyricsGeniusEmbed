...
def embed_lyrics(lyrics_content,album_directory,song_title, song_titles):
    try:
        songs_dir = ['{}/{}'.format(album_directory,file_name) for file_name in os.listdir(album_directory) if file_name.endswith('.mp3')] # full songs file and directory
        lyrics_content = '\n'.join(lyrics_content)
        
        lyrics_content = lyrics_content.encode('latin-1', 'ignore')
        lyrics_content = lyrics_content.decode('utf8')

        for song,song_dir in zip(song_titles, songs_dir):
            if song_title == song:
...
def main():
    ...
    # arg = argparse.Namespace(directory=True, directory_name='/home/work/Music/Nick Jonas - Essentials (2020)')  # for debug 
    ...
    song_titles = [eyed3.load(i).tag.title for i in song_files]
        ...
        for song_lyrics_url in song_lyrics_urls:
            try:
                ...
                if soup.find('title').get_text() == 'Burrr! | Genius':
                    ...
                    song_title = song_title[song_title.index('– ')+2:song_title.index("Lyrics")].strip()
                    print(f'Downloading {song_title}')
                    lyrics = soup.find('div', class_='lyrics').find('p').getText().split('\n')
                    embed_lyrics(lyrics, arg.directory_name, song_title, song_titles)
                time.sleep(7)
            except Exception as ex:
                print(ex)
    else:
        ...
        if arg.album:
            ...
            if not soup.find('title').get_text() == 'Burrr! | Genius':
                ...
                for link, s_title in zip(link_ls,son_titles):
                    ...
                    soup = BeautifulSoup(req.text,'html.parser')
                    print(f'Downloading {s_title}')
                    lyrics = soup.find('div', class_='lyrics').find('p').getText().split('\n')
                    embed_lyrics(lyrics, album_location, s_title, song_titles)
                    time.sleep(7)
            else:
